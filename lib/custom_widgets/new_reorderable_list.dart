// I wanted to just extend classes in the reorderable_list.dart file
// but since there are a lot of private methods I just have to bring the whole
// file over and change two small things. add a _sizeOfItemAsOffset method
// and change the _dragEnd method
...
class ReorderableListState extends State<ReorderableList> {
  final GlobalKey<SliverReorderableListSizesDifferState>
      _sliverReorderableListSizesDifferKey = GlobalKey();
  ...
  void startItemDragReorder({
    ...
    _sliverReorderableListSizesDifferKey.currentState!.startItemDragReorder(
  ...
  void cancelReorder() {
    _sliverReorderableListSizesDifferKey.currentState!.cancelReorder();
  }
    ...
    return CustomScrollView(
      ...
      slivers: <Widget>[
        SliverPadding(
          ...
          sliver: SliverReorderableListSizesDiffer(
            key: _sliverReorderableListSizesDifferKey,
...
///  * [ReorderableListView], a material design list that allows the user to
///    reorder its items.
class SliverReorderableListSizesDiffer extends StatefulWidget {
  /// Creates a sliver list that allows the user to interactively reorder its
  /// items.
  ///
  /// The [itemCount] must be greater than or equal to zero.
  const SliverReorderableListSizesDiffer({
    Key? key,
    required this.itemBuilder,
    required this.itemCount,
    required this.onReorder,
  ...
  @override
  SliverReorderableListSizesDifferState createState() =>
      SliverReorderableListSizesDifferState();
  ...
  static SliverReorderableListSizesDifferState of(BuildContext context) {
    assert(context != null);
    final SliverReorderableListSizesDifferState? result = context
        .findAncestorStateOfType<SliverReorderableListSizesDifferState>();
    assert(() {
      if (result == null) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ErrorSummary(
  ...
  ///    ancestor is found.
  static SliverReorderableListSizesDifferState? maybeOf(BuildContext context) {
    assert(context != null);
    return context
        .findAncestorStateOfType<SliverReorderableListSizesDifferState>();
  }
...
class SliverReorderableListSizesDifferState
    extends State<SliverReorderableListSizesDiffer>
  ...
  @override
  void didUpdateWidget(covariant SliverReorderableListSizesDiffer oldWidget) {
  ...
  // * Lance Harris: to help handle the case of dropping as the last item.
  // * previously it took the item height in its calculations (extent), but that
  // * assumes items are of the same size. instead grab the height of the second
  // * to last item of the list
  Offset _sizeOfItemAsOffset(int index, Axis scrollDirection) {
    final RenderBox itemRenderBox =
        _items[index]!.context.findRenderObject()! as RenderBox;
    switch (scrollDirection) {
      case Axis.horizontal:
        return Offset(itemRenderBox.size.width, 0.0);
      case Axis.vertical:
        return Offset(0.0, itemRenderBox.size.height);
    }
  }

  void _dragEnd(_DragInfo item) {
    setState(() {
      // * Lance Harris: I changed widget.itemCount - 1 to widget.itemCount since
      // * the previous version didn't only target dropping as the last item
      // * but also as the second to last item
      if (_insertIndex! < widget.itemCount) {
        ...
        if (_reverse) {
          _finalDropPosition = _itemOffsetAt(itemIndex) -
              _sizeOfItemAsOffset(itemIndex, _scrollDirection);
          // _extentOffset(item.itemExtent, _scrollDirection); // previous code
        } else {
          // * LH: we should be adding the extent of the previous item not the
          // * current item. It only matters if we have different sized items.
          _finalDropPosition = _itemOffsetAt(itemIndex) +
              _sizeOfItemAsOffset(itemIndex, _scrollDirection);
          // _extentOffset(item.itemExtent, _scrollDirection); // previous code
        }
...
class _ReorderableItemState extends State<_ReorderableItem> {
  late SliverReorderableListSizesDifferState _listState;
  ...
  @override
  void initState() {
    _listState = SliverReorderableListSizesDiffer.of(context);
...
class ReorderableDragStartListener extends StatelessWidget {
  ...
  void _startDragging(BuildContext context, PointerDownEvent event) {
    final SliverReorderableListSizesDifferState? list =
        SliverReorderableListSizesDiffer.maybeOf(context);
...
class _DragInfo extends Drag {
  ...
  late SliverReorderableListSizesDifferState listState;
  late int index;
  late Widget child;
...
class _DragItemProxy extends StatelessWidget {
  ...
  final SliverReorderableListSizesDifferState listState;
  final int index;
  final Widget child;
...
class _ReorderableItemGlobalKey extends GlobalObjectKey {
  ...
  final Key subKey;
  final int index;
  final SliverReorderableListSizesDifferState state;
...
// from reorderable_lsit.dart of the material folder
class ReorderableListSizesDiffer extends StatefulWidget {
  ...
  const ReorderableListSizesDiffer.builder({
    Key? key,
    required this.itemBuilder,
    required this.itemCount,
    required this.onReorder,
  ...
  @override
  State<ReorderableListSizesDiffer> createState() =>
      _ReorderableListSizesDifferState();
}

class _ReorderableListSizesDifferState
    extends State<ReorderableListSizesDiffer> {
